        ...
        adw::ActionRow {
            ...
            connect_activated[sender, index] => move |_| {
                sender.output(NetworkRowOutput::Connect(
                    index.current_index().to_string() // we'll fix this below
                )).ok();
            }
        }
    }
    ...
    fn update(&mut self, message: Self::Input, sender: ComponentSender<Self>) {
        match message {
            ...
            WifiInput::ToggleWifi(on) => {
                ...
                // Clear the list immediately when turning off
                if !on {
                    let mut guard = self.networks.guard();
                    guard.clear();
                }
                ...
                relm4::spawn_local(async move {
                    ...
                    if on {
                        // Wait for NM to bring the adapter up and do an initial scan
